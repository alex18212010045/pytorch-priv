...
def train(loader, model, criterion, optimizer, epoch, use_cuda):
    global BEST_ACC, LR_STATE, premodel
    ...
    top1 = AverageMeter()
    top5 = AverageMeter()
    tea_top1 = AverageMeter()
    tea_top5 = AverageMeter()
    end = time.time()
    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        #teacher_acc
        tea_prec1, tea_prec5 = accuracy(outputs.data, targets.data, topk=(1, 5))
        tea_top1.update(tea_prec1[0], inputs.size(0))
        tea_top5.update(tea_prec5[0], inputs.size(0))
        ...
        if (batch_idx + 1) % cfg.CLS.disp_iter == 0:
            print('Training: [{}/{}][{}/{}] | Best_Acc: {:4.2f}% | Time: {:.2f} | Data: {:.2f} | '
                  ...
                  .format(epoch + 1, cfg.CLS.epochs, batch_idx + 1, len(loader), BEST_ACC, batch_time.average(),
                          data_time.average(), LR_STATE, top1.avg, top5.avg, tea_top1.avg, tea_top5.avg, losses.avg,
                          batch_time.sum + data_time.sum, drop_prob))
...
def test(val_loader, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(val_loader):
        ...
        print('Testing: [{}/{}][{}/{}] | Best_Acc: {:4.2f}% | Time: {:.2f} | Data: {:.2f} | '
              'LR: {:.8f} | Top1: {:.4f}% | Top5: {:.4f}% | Teacher Top1: {:.4f}% | Teacher Top5: {:.4f}% | Loss: {:.4f} | Total: {:.2f}'
              .format(epoch + 1, cfg.CLS.epochs, batch_idx + 1, len(val_loader), BEST_ACC,
                      batch_time.average(), data_time.average(),
                      LR_STATE, top1.avg, top5.avg, tea_top1.avg, tea_top5.avg, losses.avg, batch_time.sum + data_time.sum))
...
def main():
 ...
 # Dataset and Loader
    ...
    model = models.__dict__[cfg.CLS.arch](drop_prob=cfg.CLS.drop_prob, block_size=cfg.CLS.block_size, premodel=premodel, nr_steps=cfg.CLS.nr_steps)
    print(model)
