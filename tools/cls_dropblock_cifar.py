    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        if (batch_idx + 1) % cfg.CLS.disp_iter == 0:
            print('Training: [{}/{}][{}/{}] | Best_Acc: {:4.2f}% | Time: {:.2f} | Data: {:.2f} | '
                  'LR: {:.8f} | Top1: {:.4f}% | Top5: {:.4f}% | Loss: {:.4f} | Total: {:.2f} | drop_prob: {:.5f}'
...
def save_checkpoint(model, optimizer, cur_acc, epoch):
    ...
    try:
        # add time.sleep to avoid model fails to save in ava. By Riheng 2018/12/06
        time.sleep(4)
...
def main():
    ...
    model = models.__dict__[cfg.CLS.arch](drop_prob=cfg.CLS.drop_prob, block_size=cfg.CLS.block_size, nr_steps=cfg.CLS.nr_steps)
    print(model)
